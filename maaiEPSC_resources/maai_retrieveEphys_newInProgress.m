function [ data, loaded] = maai_retrieveEphys( ephysFltr, dataType, dataPath)
...
%Loop over names and retrieve data
for i = 1:numel(dataNames)
    if exist(fullfile(dataPath, [dataNames{i},'.mat']),'file') == 2
        dataFile = matfile(fullfile(dataPath, [dataNames{i},'.mat']));    
        
         varList = who(dataFile);
         varList=varList{1};
%          fieldnames(dataFile,varList)

        
        
        loaded(i,:) = ismember(dataType,varList);
        for j = find(loaded(i,:))
            data{i,j} = dataFile.(dataType{j});
        end
    else
        disp([dataPath, dataNames{i},'.mat', ' does not exist']);
    end
end
...
length(dataFile.(varName))
fieldnames(dataFile.(varName))


if size(data,1)*size(data,2)==1
    data=data{1,1};
end
